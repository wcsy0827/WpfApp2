...
namespace PhotoDataModel
{
    ...
    {
        public  List<PhotoDataItem> Search(string SearchTerm = "Rivers",     int results = 50)
        {
       ...
       async   public  Task < List<PhotoDataItem>> SearchAsync(string SearchTerm = "Rivers", int results = 50)
        {
            ...
            List<PhotoDataItem> list = photoQuery.ToList();// await Task.Run(() => photoQuery.ToList()); //呼叫任意方法- 用 Task 加 await 會在背景做 看Timer
            return list;
        }

       async public Task<List<PhotoDataItem>> SearchAsync2(string SearchTerm = "Rivers", int results = 50)
       {
           ...
           List<PhotoDataItem> list = await Task.Run(() => photoQuery.ToList()); //呼叫任意方法- 用 Task 加 await 會在背景做 看Timer
           return list;
       }
